...
public class UpcomingPagingSource extends RxPagingSource<Integer, Movie> {
    @NotNull
    @Override
    public Single<LoadResult<Integer, Movie>> loadSingle(@NotNull LoadParams<Integer> loadParams) {
        ...
        return HttpClient.getInstance()
                .getUpcomingMovieList(APIKey.getInstance().getTMDB_API_KEY(), pageNo)
                .subscribeOn(Schedulers.io())
                .map(MovieResponse::getMovieList)
                .map(upcomingMovieList -> toLoadResult(upcomingMovieList, pageNo))
                .onErrorReturn(LoadResult.Error::new);

    }

    public LoadResult<Integer, Movie> toLoadResult(List<Movie> upcomingMovieList, Integer page) {
        return new LoadResult.Page(upcomingMovieList, page == 1 ? null : page - 1, page + 1);
    }
